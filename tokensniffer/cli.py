...
@click.option("--include-metrics", is_flag=True)
@click.option("--include-tests", is_flag=True)
@click.option("--block-until-ready", is_flag=True)
@click.pass_obj
def get_token(
    api, chain_id, address, include_metrics, include_tests, block_until_ready
):
    result = api.get_token(
        chain_id, address, include_metrics, include_tests, block_until_ready
    )
    click_echo_json(result)
